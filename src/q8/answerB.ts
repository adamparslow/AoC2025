...
function main() {
  ...
  const circuits: string[][] = coords.map(val => [makeKey(val)]);
  let firstFlag = 0;
  ...
  for (let distance of distances) {
    ...
    // console.log({ firstCircuit, secondCircuit, first, second })
    ...
    if (firstFlag >= 10) {
      if (length === 1) {
        console.log("length", { length, first, second, answer: parseInt(first.split(",")[0]) * parseInt(second.split(",")[0]) });
        break;
      }
    } else {
      firstFlag++; 
    ...
    console.log("circuits length", { length: circuits.length, first, second, circuits })
  }
